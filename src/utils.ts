...
export function generateFilesForRepo(url: string): { paths: string[]; isTruncated: boolean } {
  ...
  } else if (repoName.toLowerCase().includes('express') || repoName.toLowerCase().includes('stripe') || repoName.toLowerCase().includes('billing')) {
    paths.push(
      '.env.example',
      'lib/express.js',
      'lib/router/index.js',
 ...
 */
export function scanFilesForFindings(paths: string[], runId: string, customRules: PolicyRule[] = DEFAULT_RULES): VerifiedFinding[] {
